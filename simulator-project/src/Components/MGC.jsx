...
const MGC = () => {
  ...
  const [alpha, setAlpha] = useState(''); // Mean (for gamma)
  const [beta, setBeta] = useState(''); // Variance (for gamma)
  ...
  const calculateMetrics = () => {
    ...
    if (distribution === 'uniform') {
      ...
      if (!alpha || !beta || parseFloat(alpha) <= 0 || parseFloat(beta) <= 0) {
        alert('Please enter valid mean and variance values (both > 0)');
        return;
      }
    }
    ...
    switch (distribution) {
      ...
      case 'gamma':
        // Gamma is parameterized directly by mean and variance here (same as
        // Normal, and same as GGC.jsx / the backend DistributionHelper),
        // NOT by shape/rate — so no alpha/beta conversion is needed.
        const meanGamma = parseFloat(alpha);
        const varGamma = parseFloat(beta);
        meanServiceTime = meanGamma;
        cs2 = varGamma / Math.pow(meanGamma, 2);
        break;
      ...
      <div className="max-w-6xl mx-auto mb-8">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="flex border-b">
            <button
              onClick={() => setTab("form")}
              className={`flex-1 py-4 text-center font-bold text-lg transition ${tab === "form"
            ...
            <button
              ...
              className={`flex-1 py-4 text-center font-bold text-lg transition ${tab === "results"
