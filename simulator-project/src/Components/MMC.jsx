...
const MMC = () => {
  ...
  // ---- Excel / simulation mode state ----
  const [excelInputs, setExcelInputs] = useState(null); // { arrivalTimes, serviceTimes, priorities } | null
  const [dataSource, setDataSource] = useState("random"); // "random" | "excel"
  const [simResult, setSimResult] = useState(null); // result from runQueueSimulation
          ...
          {tab === "form" && (
            <div className="p-8">
                ...
                {excelInputs && (
                  <div className="mt-2 flex items-center justify-between bg-[#2C80D3]/10 border border-[#2C80D3]/40 rounded-xl px-5 py-3">
                    <span className="text-sm font-semibold text-[#0C3E72]">
                      ✅ Excel data loaded — {excelInputs.arrivalTimes.length} patients. Clicking &quot;Calculate&quot; will run a multi-server simulation instead of the analytical formula.
                    </span>
                    <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                {/* Arrival Rate — hidden when Excel is loaded (λ comes from Excel) */}
                {!excelInputs && (
                  <div className="group">
                ...
                {/* Service Rate — hidden when Excel is loaded */}
                {!excelInputs && (
                  <div className="group">
                    <label className="block text-sm font-bold text-[#091d3a] uppercase tracking-wider mb-3">
                      Service Rate (μ)
                ...
                {/* Number of Servers — always shown */}
                <div className="group">
                  <label className="block text-sm font-bold text-[#091d3a] uppercase tracking-wider mb-3">
                    Number of Servers (C)
                  </label>
              ...
              <p className="mt-4 text-center text-sm font-medium text-[#0C3E72]">
                {excelInputs
                  ...
                  : 'Will compute analytical M/M/C queueing metrics from the parameters above.'}
              </p>
            </div>
          )}
              ...
              <div className="mb-8 p-6 bg-gradient-to-r from-gray-50 to-white rounded-2xl shadow border border-gray-200">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-2xl font-bold text-[#0C3E72]">{results.mu}</div>
                  </div>
                  <div className="text-center">
              ...
              <div className="bg-gradient-to-r from-white to-gray-50 rounded-2xl shadow-lg border border-gray-200 p-6 mb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {[
                        { label: "Avg Wait in Queue", value: results.Wq, color: "text-[#2C80D3]" },
                        { label: "Avg System Time", value: results.W, color: "text-[#0C3E72]" },
                        { label: "Queue Length", value: results.Lq, color: "text-[#091d3a]" },
                        { label: "System Population", value: results.L, color: "text-[#2C80D3]" }
                      ].map((item, idx) => (
                ...
                <div className="mb-10 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200 shadow">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-3 bg-white/70 rounded-lg border">
                      <div className="font-semibold text-[#2C80D3] mb-1">1. Utilization Factor</div>
                    ...
                    <div className="p-3 bg-white/70 rounded-lg border">
                      <div className="font-semibold text-[#0C3E72] mb-1">2. Probability of Zero Customers</div>
                      <div className="text-sm font-mono">P₀ = 1 / [∑ (Cρ)ⁿ/n! + (Cρ)^C/(C!(1-ρ))]</div>
                    </div>
                    <div className="p-3 bg-white/70 rounded-lg border">
                      <div className="font-semibold text-[#091d3a] mb-1">3. Average Queue Length</div>
                      <div className="text-sm font-mono">Lq = (P₀ × (λ/μ)^C × ρ) / (C! × (1-ρ)²)</div>
                    </div>
                    <div className="p-3 bg-white/70 rounded-lg border">
                      <div className="font-semibold text-[#2C80D3] mb-1">4. Average System Length</div>
                      <div className="text-sm font-mono">L = Lq + λ/μ</div>
                    </div>
                    <div className="p-3 bg-white/70 rounded-lg border">
                      <div className="font-semibold text-[#0C3E72] mb-1">5. Average Waiting Time</div>
                      <div className="text-sm font-mono">Wq = Lq / λ</div>
                    </div>
                    <div className="p-3 bg-white/70 rounded-lg border">
                      <div className="font-semibold text-[#091d3a] mb-1">6. Average System Time</div>
...
- Idle Probability: ${results.idle}
        ...
        {tab === "simulation" && simResult && (
          <div className="p-6">
            <h2 className="text-2xl font-bold text-center text-[#0C3E72] mb-2">Simulation Results — M/M/C</h2>
            <p className="text-center text-sm text-gray-500 mb-6">Data source: Excel upload · {simResult.table.length} patients · {parseInt(servers) || 1} server(s) · Utilization: {simResult.utilization}%</p>
